...
from loader_utils import Normalize, load_npz, load_npz_2, largest_connected_components, train_val_test_split_tabular
...
''' new added funciton '''
def load_new_data_2(dataf, dataset_str="cora",norm_type='SymNorm_tildeA',cuda=True, shuffle=False, identity_features=False):
    """
    Load Citation Networks Datasets.
    """
    file_name = dataf+"g2g/"+dataset_str+".npz"
    _A_obs, _X_obs, _z_obs, graph_additional_package = load_npz_2(file_name)
    _A_obs = _A_obs + _A_obs.T
    _A_obs[_A_obs > 1] = 1
    lcc = largest_connected_components(_A_obs)
    ...
    if idx_to_node:
        ...
        for i in range(len(lcc)):
            new_idx_to_node.append(idx_to_node[lcc[i]])
        graph_additional_package["idx_to_node"] = new_idx_to_node
    if cuda:
        features = features.cuda()
        adj = adj.cuda()
        labels = labels.cuda()
