...
def getShortestPathDistance(graph_in, idx_train, labels):
    #graph_in = {
    ...
    shortest_path_list = getShortestPathDistanceNodes(node_num, cora_gcn_neighbor_set, idx_train, labels)
    return shortest_path_list
def getSavePath(dataf, data_name):
    return dataf+"/{}_SPD.pkl".format(data_name)
...
def getKFSSavePath(dataf, data_name):
    return dataf+"/{}_KFS_V1.pkl".format(data_name)
def getKFSJson(features, idx_train, labels, dataf, data_name, k=5):
    ...
    if existence:
        ...
        KFS = getTopkFeatureSimilaritySet(features, idx_train, labels, k)
        saveJson(KFS, savePath)
        return KFS
def getLayoutSavePath(dataf, data_name):
    return dataf+"/{}_Layout.pkl".format(data_name) 
def getLayout(graph_in):
    senders = graph_in["senders"]
    receivers = graph_in["receivers"]
    node_num = graph_in["node_num"]
    G = nx.Graph()
    edge_num = len(senders)
    edge_index = [(senders[i], receivers[i]) for i in range(edge_num)]
    ...
    return newPos
def getLayoutJson(graph_in, dataf, data_name):
    savePath = getLayoutSavePath(dataf, data_name)
    existence = os.path.isfile(savePath)
    if existence:
        return loadJson(savePath)
    else:
        newPos = getLayout(graph_in)
        saveJson(newPos, savePath)
        return newPos
